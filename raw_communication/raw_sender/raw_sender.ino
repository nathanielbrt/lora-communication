...
void setup() {
  ...
  if (c.status.code == E22_SUCCESS) {
    ...
    Serial.println("LoRa Sender (Unreliable) configured. Address: 0x" + String(SENDER_ADDRESS, HEX));
    c.close();
  }
...
void send_data_packet(uint16_t id) {
  ...
  // Fire-and-forget: Send the packet and do not wait for a response.
  e22.sendFixedMessage(highByte(RECEIVER_ADDRESS), lowByte(RECEIVER_ADDRESS), LORA_CHANNEL, &packet, sizeof(packet));

  // Log the sent packet immediately.
  salvarDados(packet);

  packet_id_counter++;
  delay(1000); // Wait 1 second before sending the next packet to avoid flooding.
}
