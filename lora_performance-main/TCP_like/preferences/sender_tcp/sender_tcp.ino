...
void setup() {
  ...
  preferencias.begin("loraSender", false); // Namespace "loraSender", modo read/write
...
void loop() {
  ...
  while (!ack_received && attempts <= MAX_RETRANSMISSIONS) {
    ...
    e220.sendMessage((uint8_t*)&packet, sizeof(DataPacket)); // Tamanho fixo!
    ...
    while (!ack_received && millis() - start_time < ACK_TIMEOUT) {
      ...
      yield(); // Libera tempo para o sistema (WDT e tarefas RTOS)
      delay(1); // Reduz uso da CPU
    }
